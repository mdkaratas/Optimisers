...
def update_nodes(xvals_list, fvals_post , fvals_list, nodes_list, threshold):
    edge_indices = []
    xvals = np.array(xvals_list)  #post yerin elist yaz
...
for k,v in st.items():    
    ...
    local_edges = [(c, idx, count[j] * 1/((2**ndim)*int(v.shape[0]/3)) ) for j, idx in enumerate(uniqe_edge_indices)]
