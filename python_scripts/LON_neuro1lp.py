...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 09:32:22 2021
...
# constraint penalty added via distance 
...
comb = list(zip(nodes_list,fvals_list))
sorted_list = sorted(comb, key=lambda x: x[1])
sorted_list = sorted_list[0:77]
xlist = []
flist =[]
...
for k,v in st.items():
    ...
    for i in range(2**ndim): 
        init_simplex[1:] = init_simplex[0] + init_simplex[0]*permutations[i].reshape(-1,1)* step
        OptimizeResult = minimize(neuro1lp, v[0], method='nelder-mead', 
                                  options={'initial_simplex': init_simplex, 'xatol': 1e-8, 'disp': False})
