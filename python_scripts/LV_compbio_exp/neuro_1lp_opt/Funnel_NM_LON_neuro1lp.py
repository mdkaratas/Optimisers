...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  3 17:28:54 2022
...
for p in range(len(nodes_list)): 
    ...
    if not p in recal:
        ...
        weight = nm[:,2]
        norm_weight = [float(num)/sum(weight) for num in weight]
        ...
        print("--- %s seconds ---" % (time.time() - start_time))     
        for j in range(len(global_edges)):
            while not global_edges[j][0]==global_edges[j][1]:
                if not global_edges[j][1] in recal:
                    start_time = time.time()
                    recal.append(global_edges[j][1])
                    init_simplex[0] = nodes_list[global_edges[j][1]]
                    for k in range(2**2): 
                        init_simplex[1:] = init_simplex[0] + init_simplex[0]*permutations[k].reshape(-1,1)* step
                        OptimizeResult = minimize(neuro1lp, nodes_list[global_edges[j][1]], method='nelder-mead', 
                                                  options={'initial_simplex': init_simplex, 'xatol': 1e-8,'fatol': 1e-13})
                        xvals_post.append(OptimizeResult.x) 
                        fvals_post.append(OptimizeResult.fun)
                    edge_indices = update_nodes(xvals_post, fvals_post, fvals_list, nodes_list, threshold)
                    uniqe_edge_indices = list(set(edge_indices))
                    count = [edge_indices.count(elem) for elem in uniqe_edge_indices] # to ensure edge indices are unique
                    local_edges = [(global_edges[j][1], idx, count[c]* 1/(2**ndim)) for c,idx in enumerate(uniqe_edge_indices)] #* 1/((2**ndim)*len(nodes_list))
                    nm= np.array(local_edges)
                    weight = nm[:,2]
                    ...
                    print(local_edges)    
                    global_edges = global_edges + local_edges   
                print("--- %s seconds- new funnel coming ---" % (time.time() - start_time)) 
