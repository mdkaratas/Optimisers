...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 30 17:07:00 2022
...
#%%  
    ...
    with open(f"Desktop/design_dict_{savename}_NMMSO_cts.txt", "wb") as fp:   
     pickle.dump(design_dict, fp)  
    with open(f"Desktop/design_dict_{savename}_NMMSO_cts.txt", "rb") as fp:   
     design_dict_NMMSO_MI_P = pickle.load(fp)   
        
    with open(f"Desktop/fit_dict_{savename}_NMMSO_cts.txt", "wb") as fp:   
     pickle.dump(fit_dict, fp)   
    with open(f"Desktop/fit_dict_{savename}_NMMSO_cts.txt", "rb") as fp:   
     fit_dict_NMMSO_MI_P = pickle.load(fp)     
        
    
    with open(f"Desktop/x_NMMSO_{savename}_NMMSO_cts.txt", "wb") as fp:   #Pickling
     pickle.dump(x_NMMSO_MI, fp) 
     
    with open(f"Desktop/x_NMMSO_{savename}_NMMSO_cts.txt", "rb") as fp:   # Unpickling
     x_NMMSO_MI_P = pickle.load(fp)
    
    with open(f"Desktop/f_NMMSO_{savename}_NMMSO_cts.txt", "wb") as fp:   #Pickling
     pickle.dump(f_NMMSO_MI, fp)   
    with open(f"Desktop/f_NMMSO_{savename}_NMMSO_cts.txt", "rb") as fp:   # Unpickling
